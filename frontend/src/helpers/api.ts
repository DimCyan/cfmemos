...
export function signup(username: string, password: string) {
  return axios.post<User>("/api/v1/auth/signup", {
    username,
    password,
    nickname: username, // 使用 username 作为默认 nickname
  });
}
...
export function getMemoList(memoFind?: MemoFind) {
  const queryList = [];
  if (memoFind?.creatorUsername) {
    queryList.push(`creatorUsername=${encodeURIComponent(memoFind.creatorUsername)}`);
  }
  if (memoFind?.rowStatus) {
    queryList.push(`rowStatus=${encodeURIComponent(memoFind.rowStatus)}`);
  }
  if (memoFind?.pinned) {
  ...
  // 搜索参数
  if (memoFind?.text) {
    queryList.push(`text=${encodeURIComponent(memoFind.text)}`);
  }
  if (memoFind?.tag) {
    queryList.push(`tag=${encodeURIComponent(memoFind.tag)}`);
  }
  if (memoFind?.dateFrom) {
    queryList.push(`dateFrom=${memoFind.dateFrom}`);
  }
  if (memoFind?.dateTo) {
    queryList.push(`dateTo=${memoFind.dateTo}`);
  }
...
export function createResourceWithBlob(formData: FormData) {
  return axios.post<Resource>("/api/v1/resource", formData);
}
